...
def plot_hex_grid(N, numbers):
    ...
    # Calculate dynamic font size based on the radius N
    base_font_size = 12  # Default font size for smaller grids
    dynamic_font_size = max(base_font_size * (7 / N), 4)  # Increase the font size slightly
    ...
    for row, row_numbers in enumerate(numbers):
        ...
        for col, num in enumerate(row_numbers):
            ...
            # Add the number if it's not zero, with dynamic font size
            if num != 0:
                ax.text(x, y, str(num), ha='center', va='center', fontsize=dynamic_font_size)
